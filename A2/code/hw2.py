...
image_name_set = ['notre', 'mount', 'gaudi']
image_name = 'gaudi'

if image_name == image_name_set[0]:
    # Notre Dame
...
elif image_name == image_name_set[1]:
...
elif image_name == image_name_set[2]:
...
'''
# cheat_interest_points function returns the numpy array with shape (k, 1).
# The function explanation is wrong in this function. It should be flattened.
# Also, gradient suppression should be applied.
delta = feature_width // 2
x1 = x1.flatten()
y1 = y1.flatten()
x2 = x2.flatten()
y2 = y2.flatten()
x1 = x1[(x1 >= delta) & (x1 <= image1.shape[1]-delta-1)]
y1 = y1[(y1 >= delta) & (y1 <= image1.shape[0]-delta-1)]
x2 = x2[(x2 >= delta) & (x2 <= image2.shape[1]-delta-1)]
y2 = y2[(y2 >= delta) & (y2 <= image2.shape[0]-delta-1)]
'''
# plt.figure(); plt.imshow(image2_bw, cmap='gray'); plt.show()
...
image1_features = get_features(image1_bw, x1, y1, feature_width, scales=None)
image2_features = get_features(image2_bw, x2, y2, feature_width, scales=None)
